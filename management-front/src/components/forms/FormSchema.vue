    ...
    <div class="grid gap-4 mb-5">
      <template v-if="parsedTemplate.length > 0">
        <div
          ...
          <div :class="`${item.overrideGroupClass ? '' : 'form-group'} ${item.groupClass ?? ''}`">
            <label
              v-if="
                ['text', 'email', 'password', 'number', 'select', 'textarea'].includes(item.type) &&
                item.label
              "
              :for="`input-${itemIndex}`"
              :class="`${
...
interface IProps {
  template: Array<IFormSchemaItem>;
      ...
      if (item.validate) {
        ...
        if (item.validate.custom && item.validate.customFn) {
          if (item.validate.customMessage) {
            rule.custom = helpers.withMessage(item.validate.customMessage, item.validate.customFn);
          } else {
